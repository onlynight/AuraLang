...
test('client bundle: loads as a DSH lazy module and exports apply + inject', () => {
  ...
  assert.equal(id, '@phir-lang/dsh-highlight-phir');
  ...
  const ctx = {
    ...
    documentPreviews: {
      register(definition) {
        ...
        assert.equal(JSON.stringify(definition.extensions), '["phir"]');
        // Complete bytes: the paged text read refuses any page holding a NUL
        // byte, which a `.phir` IR dump with a binary payload always trips.
