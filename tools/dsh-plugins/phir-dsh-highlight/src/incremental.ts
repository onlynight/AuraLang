 ...
 * @param lang - language id. Default `PHIR`.
...
export function tokenizeNextChunk(
  ...
  // Enforce the line budget by re-cutting at the end of an earlier line.
 ...
 *
 * The result always sits just after a newline (or at the end of the document),
 * never before one: a chunk that ended *at* a newline would leave the next
 * chunk starting with one, which `codeToTokens` reports as a real first line
 * and the accumulated output would grow a blank row per boundary.
 *
 * The caller reaches this only when the character budget already covers more
 * than `maxLines` lines, so the `maxLines`-th break is guaranteed to exist
 * inside the range being replaced and the result always shrinks it.
 *
 * @param code - full source text.
 * @param from - cursor: the first offset not yet consumed.
 * @param maxLines - line budget for this call.
 * @returns the offset to stop the chunk at.
 */
