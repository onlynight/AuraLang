 ...
 * PHIR grammar and both PHIR themes.
 *
...
import { PHIR_GRAMMAR } from './phir-grammar.js';
import { createEngine, type EngineOptions } from './engine.js';
import { PHIR_DARK_THEME, PHIR_LIGHT_THEME, THEME_BY_MODE } from './theme.js';
import type { ThemeMode } from './theme.js';

/** The highlighter instance this plugin owns. */
export type PHIRHighlighter = Awaited<
...
let instance: PHIRHighlighter | null = null;
let pending: Promise<PHIRHighlighter> | null = null;
...
export function getHighlighter(
  ...
  const build = async () => {
    ...
    const highlighter = await createHighlighterCore({
      engine,
      // PHIR_GRAMMAR is structurally a LanguageRegistration; the cast hides the
      // local TextmateGrammar index signature from Shiki's exact types.
      langs: [PHIR_GRAMMAR as unknown as LanguageRegistration],
      themes: [PHIR_LIGHT_THEME, PHIR_DARK_THEME],
      warnings: false,
    });
 ...
 */
export async function tokenizePHIR(
  code: string,
  options: HighlightOptions = {},
): Promise<ThemedToken[][]> {
 ...
 */
export async function tokenizePHIRWithState(
  code: string,
  options: HighlightOptions = {},
