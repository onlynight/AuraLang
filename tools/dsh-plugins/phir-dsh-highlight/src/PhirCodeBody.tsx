/**
 * The PHIR document body: an incrementally highlighted code view for the
...
import type { PHIRHighlighter } from './highlighter.js';
import { getHighlighter, resolveThemeMode } from './highlighter.js';
import { decodePHIRSource } from './decode.js';
...
/**
 * Hard render cap, the safety net behind the tokenizer budget: a `bytes`
 * document arrives whole, and rendering every row is the expensive half.
 * Set high on purpose - a compiler IR dump is legitimately long, and truncating
 * a real file is worse than a slow pane - but it is not free: each row is two
 * DOM nodes, so a document this long means a long first frame and a very large
 * tree.
 */
const MAX_LINES = 500_000;

export type PhirCodeBodyProps = DocumentPreviewProps &
  PropsLocale<typeof LOCALE_NAMESPACE>;

export function PhirCodeBody({
  content,
  wrap,
  scrollportRef,
  t,
}: PhirCodeBodyProps): ReactNode {
  const [phase, setPhase] = useState<Phase>('warming');
  const [lines, setLines] = useState<CodeLines>(null);
  const [dark, setDark] = useState(() => readDark());
  const [copied, setCopied] = useState(false);

  const highlighterRef = useRef<PHIRHighlighter | null>(null);
  ...
  const text: string = useMemo(
    () => (typeof source === 'string' ? source : decodePHIRSource(source)),
    [source],
  );
  ...
  useEffect(() => {
    if (phase === 'failed') {
      ...
      const palette = dark ? PHIR_DARK_PALETTE : PHIR_LIGHT_PALETTE;
  ...
  return (
    <div data-phir-code data-code-preview data-wrap={wrap ? 'true' : 'false'}>
      <div data-phir-banner>
        <span data-phir-lang>PHIR</span>
        <button
          type="button"
          data-phir-copy
      ...
      <div data-phir-scroll ref={scrollportRef}>
        <div data-phir-grid>
        ...
        {sourceLines.length === 0 ? <div data-phir-empty> </div> : null}
      </div>
    </div>
  );
}
...
function FragmentPair({
  ...
  return (
    <>
      <span data-phir-num data-textpreview-line={num}>{num}</span>
      <span data-phir-text>{tokens === null ? raw : renderTokens(tokens)}</span>
    </>
  );
}
