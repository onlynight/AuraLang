 ...
 *
 * The registration asks for `text-pages` (see `client.ts`), so this path is the
 * unused half of the body's contract by default - but it is the half that
 * matters for PHIR specifically. PHIR is a compiler IR, and an IR dump is
 * commonly a textual header followed by a binary payload. DSH's *paged* text
 * read refuses any page whose text contains a NUL byte
 * (`workspace-file/not-text`) and its filesystem backend rejects malformed
 * UTF-8, so such a file cannot be read at all while the owner streams pages: the
 * pane reports a non-text file before any renderer is consulted. Declaring
 * `loading: 'bytes-complete'` routes the same file through here instead, where
 * it decodes:
 *
 ...
 */
export function decodePHIRSource(data: Uint8Array): string {
