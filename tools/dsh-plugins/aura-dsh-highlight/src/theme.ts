/**
 * Aura color themes for Shiki.
 *
 * Two themes (`aura-light`, `aura-dark`) are built from one shared rule table
 * over one shared color vocabulary. Every color is emitted as a CSS variable
 * reference with a literal fallback:
 *
 *   var(--aura-keyword, #d6336c)
 *
 * That single choice does three jobs:
 *
 *   1. The viewer can redefine `--aura-*` on its own element (or on `:root`,
 *      which is where this plugin installs them) and every token follows.
 *   2. DSH's existing `--dsw-*` tokens can be aliased onto `--aura-*` by a
 ...
 *
 * Scope names follow the grammar's own `.aura` suffix convention
 * (see grammar/aura.tmLanguage.json).
 */
...
/** Light palette: VSCode Light+ hue identity, matching the Aura VSCode plugin. */
export const AURA_LIGHT_PALETTE: Palette = {
  foreground: '#1f2328',
  comment: '#6a737d',
  'comment-doc': '#6a737d',
  keyword: '#d6336c',
...
/** Dark palette: VSCode Dark+ hue identity, matching the Aura VSCode plugin. */
export const AURA_DARK_PALETTE: Palette = {
  foreground: '#e6edf3',
  comment: '#8b949e',
  'comment-doc': '#8b949e',
  keyword: '#ff7b72',
...
export const AURA_LIGHT_THEME: ThemeRegistration = buildTheme(
  'aura-light',
  'light',
  AURA_LIGHT_PALETTE,
);

export const AURA_DARK_THEME: ThemeRegistration = buildTheme(
  'aura-dark',
  'dark',
  AURA_DARK_PALETTE,
);
...
function buildTheme(
  ...
  return {
    name,
    displayName: name === 'aura-dark' ? 'Aura Dark' : 'Aura Light',
    type,
    colors: {
 ...
 * re-render. Consumers that prefer their own colors redefine `--aura-*` on any
 * ancestor of the viewer.
 */
export function themeVariableCss(): string {
  const light = AURA_LIGHT_PALETTE;
  const dark = AURA_DARK_PALETTE;
