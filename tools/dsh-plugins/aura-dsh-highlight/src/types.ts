...
/** Capture group index → scope name (or nested capture table). */
...
export type AuraLanguage = 'aura';

/**
 * The one `.aura` extension this plugin owns.
 *
 * Held as a single constant so the document-preview registration, the path
 * matcher, and the tests cannot drift apart.
 */
export const AURA_EXTENSION = 'aura';

/** Case-insensitive aliases this plugin answers to. */
export const AURA_ALIASES: readonly string[] = ['aura'];

/**
 * Decide whether a decoded file path is an Aura source file.
 *
 * Compound suffixes are rejected: `aura.ts` is TypeScript that happens to
 * mention "aura", not an Aura source file.
 *
 * @param path - decoded path or bare filename, either slash style.
 * @returns `true` when this plugin should render the file.
 */
export function isAuraPath(path: string | undefined): boolean {
  if (!path) return false;
  const name = path.replace(/\\/g, '/').split('/').pop() ?? '';
  const dot = name.lastIndexOf('.');
  if (dot <= 0 || dot === name.length - 1) return false;
  return name.slice(dot + 1).toLowerCase() === AURA_EXTENSION;
}
