 ...
 * HAT grammar and both HAT themes.
 *
...
import { HAT_GRAMMAR } from './hat-grammar.js';
import { createEngine, type EngineOptions } from './engine.js';
import { HAT_DARK_THEME, HAT_LIGHT_THEME, THEME_BY_MODE } from './theme.js';
import type { ThemeMode } from './theme.js';

/** The highlighter instance this plugin owns. */
export type HATHighlighter = Awaited<
...
let instance: HATHighlighter | null = null;
let pending: Promise<HATHighlighter> | null = null;
...
export function getHighlighter(
  ...
  const build = async () => {
    ...
    const highlighter = await createHighlighterCore({
      engine,
      // HAT_GRAMMAR is structurally a LanguageRegistration; the cast hides the
      // local TextmateGrammar index signature from Shiki's exact types.
      langs: [HAT_GRAMMAR as unknown as LanguageRegistration],
      themes: [HAT_LIGHT_THEME, HAT_DARK_THEME],
      warnings: false,
    });
 ...
 */
export async function tokenizeHAT(
  code: string,
  options: HighlightOptions = {},
): Promise<ThemedToken[][]> {
 ...
 */
export async function tokenizeHATWithState(
  code: string,
  options: HighlightOptions = {},
