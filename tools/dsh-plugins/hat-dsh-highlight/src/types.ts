...
/** Capture group index 到 scope name (or nested capture table). */
...
export type HatLanguage = 'hat';

/**
 * The one `.hat` extension this plugin owns.
 *
 * Held as a single constant so the document-preview registration, the path
 * matcher, and the tests cannot drift apart.
 */
export const HAT_EXTENSION = 'hat';

/** Case-insensitive aliases this plugin answers to. */
export const HAT_ALIASES: readonly string[] = ['hat'];

/**
 * Decide whether a decoded file path is a HAT source file.
 *
 * Compound suffixes are rejected: `hat.ts` is TypeScript that happens to
 * mention "hat", not a HAT source file.
 *
 * @param path - decoded path or bare filename, either slash style.
 * @returns `true` when this plugin should render the file.
 */
export function isHatPath(path: string | undefined): boolean {
  if (!path) return false;
  const name = path.replace(/\\/g, '/').split('/').pop() ?? '';
  const dot = name.lastIndexOf('.');
  if (dot <= 0 || dot === name.length - 1) return false;
  return name.slice(dot + 1).toLowerCase() === HAT_EXTENSION;
}
