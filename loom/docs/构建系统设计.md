...
> **CLI 职责拆分**：第四阶段将单一 `aura` 二进制拆分为两个独立命令：
> - **`loom`**：构建编排（生命周期 / 任务引擎 / 缓存 / 插件 / 配置）
> - **`aura`**：语言语义 + 包生态（编译 / 运行 / 安装 / 发布 / 包管理）
>
> 详细职责边界见 `src/lib.rs`。

```text
# loom 构建命令（构建编排）
loom build  [--profile <p>] [--target <t>] [--parallel <n>] [--skip <phases>] [--only <phases>] [--from <phase>]
loom compile [--profile <p>] [--target <t>]
loom test    [--profile <p>] [--parallel <n>]
loom run     [--profile <p>]
loom clean   [--dir <path>]
loom resolve [--offline] [--dir <path>]
loom check   [--dir <path>]              # 语法/语义检查
loom ci      [--steps <list>]
loom watch   [--profile <p>]
loom wrapper install
loom new     <name> [--template <t>]
loom ide     [--dir <path>] [--all-files]
loom doc     [--output <dir>] [--module <name>]  # 文档生成（委托 compiler docgen）
loom fmt     [--dir <path>] [--check]            # 源码格式化（委托 compiler format_source）
loom check-config [--dir <path>]
loom version

# aura 语言/包命令（由 aura 二进制提供）
aura build <file.aura> [--output <out>]
aura run <file.aura>
aura install [--offline]
aura update [--all]
aura publish [--dir <path>]
aura deps    [--dir <path>] [--outdated]
...
| 维度 | 状态 | 说明 |
|------|------|------|
| 构建生命周期 | ✅ 已实现 | clean → resolve → compile → test → package → verify → install → deploy |
| 任务系统 | ✅ 已实现 | 任务 DAG，15 个内置任务 + 插件任务，支持并行调度 |
| 源码集 | ✅ 已实现 | main / test / bench，声明式配置，include/exclude/depends-on |
| 依赖配置 | ✅ 已实现 | 5 种分组（dependencies / compile-dependencies / runtime-dependencies / dev-dependencies / build-dependencies） |
| 插件系统 | ✅ 已实现 | 3 种类型（Convention / Explicit / External），7 个内置插件 |
| 构建配置 | ✅ 已实现 | profiles / 别名映射 / 编译选项，配置优先级（CLI > profile > project > default） |
| 构建缓存 | ✅ 已实现 | 两级缓存（本地 + 远程），fingerprint 驱动增量构建 |
| 构建包装器 | ✅ 已实现 | `aura-wrapper` 可重复构建 |
| 多项目 | ✅ 已实现 | Workspace 模式，members + default-members + 依赖图 |
| 仓库管理 | ✅ 已实现 | REST API 注册表客户端 + 本地注册表 |
| 依赖管理 | ✅ 已实现 | BOM 版本锁定 + 冲突检测 |
| IDE 集成 | ✅ 已实现 | `.loom/aura-project.json` 导出 |
| CI/CD | ✅ 已实现 | `.loom/.aura-ci.yml` 配置 + 执行器 |
| Watch 模式 | ✅ 已实现 | notify crate 事件驱动 + 增量重编 |
| 并行构建 | ✅ 已实现 | `--parallel` 参数，按执行层并行调度 |
| 输入/输出追踪 | ✅ 已实现 | fingerprint 计算 + 增量检查 |
...
> **CLI 职责拆分**（第四阶段收敛后）：
> - **`loom`**：构建编排命令（build / compile / test / run / clean / resolve / check / ci / watch / wrapper / new / ide / doc / fmt / version）
> - **`aura`**：语言语义 + 包生态命令（build / run / install / update / publish / deps / verify / package / tokens / ast / disasm / eval / repl / leak-check / lsp）
>
> `doc` 和 `fmt` 在 `loom` 中实现，委托 `compiler` 的 `docgen` / `format_source` 函数。

```text
loom
├── build               # 完整构建（clean → resolve → compile → test → package）
├── compile             # 仅编译
├── test                # 编译 + 运行测试
├── run                 # 编译 + 运行
├── clean               # 清理构建产物
...
├── new                 # 创建新项目
├── ide                 # 生成 IDE 项目文件
├── doc                 # 生成文档（委托 compiler docgen）
├── fmt                 # 格式化源码（委托 compiler format_source）
├── check-config        # 校验 aura.toml 配置
└── version             # 显示版本

aura
├── build               # 编译源码为字节码 / 原生可执行文件
├── run                 # 编译并执行
├── install             # 安装依赖
├── update              # 更新依赖
├── publish             # 发布到远程仓库
...
**后续依赖**：无（构建系统是最终集成层）
**实施状态**：Phase B1-B7 全部完成，325 个测试通过。
**CLI 拆分**：`loom`（构建编排）+ `aura`（语言语义 + 包生态），详见 §1.2。
