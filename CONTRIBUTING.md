...
| 集成测试 | `aura-compiler/tests/sema_tests.rs` | 语义检查：正确程序无诊断，错误程序报预期诊断 |
| 快照测试 | `aura-compiler/tests/snapshots.rs` | Token 流 / AST / 诊断输出的整体形态 |
| 性能基准 | `aura-compiler/tests/perf_lexer.rs` | 扫描吞吐率（release 下 > 10 MB/s） |
...
`aura-compiler/tests/snapshots/*.snap` **必须提交**；CI 会以只读方式校验。AST 结构发生有意变更时，请连同快照一起更新，并在 PR 描述中说明。
...
- 语义分析（智能转换、可见性、泛型约束、when 穷举、sealed/接口继承、重载解析等）已基本完成；新增语义检查请先在 `开发规划与实现进度.md` 的 P3 清单中认领，并在 `aura-compiler/tests/sema_tests.rs` 与 `tests/snapshots.rs` 补充测试。
