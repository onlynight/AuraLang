...
> - `docs/库导出与包格式设计方案.md` — `.auz` 制品格式与分发（本文档的后置环节）
>
...
| 包格式骨架 | `auz/mod.rs` | ⚠️ 部分实现 | 常量/错误定义，builder.rs/reader.rs 缺失 |
...
8. **与 VM 对齐**：构建系统的最终产物是 `.auz`，直接对接 VM 加载
...
| **package** | `aura package` | `package-lib`, `package-app` | 打包为 `.auz` |
...
aura build
  │
  ├─ 1. clean          → 清理 target/
  ├─ 2. resolve        → 解析依赖（Git clone / 下载 .auz）
...
| `package` | Package | `compile-main` | 打包为 `.auz` |
  ...
  "versions": [
    {
      "version": "1.2.3",
      "checksum": "sha256:abc...",
      "download": "https://registry.aura-lang.dev/v1/packages/aura-json/1.2.3/aura-json.auz",
...
├─────────────────────────────────────────────────────────┤
│                   包格式层（已有方案）                     │
│  .auz 容器 / .auc 标准格式 / 校验和 / 签名 / ModuleRegistry│
